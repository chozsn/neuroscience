...
I_injected = [-150, -100]+range(-50, 60, 10)+[100]
I_injected = [0]
...
#    area_mossy     = pi * diam_mossy * length_mossy
    ...
    g_ampa       =   0.5  * nS               # AMPA maximum conductance
    g_nmda       =   1.05*g_ampa             # NMDA maximum conductance
    ...
    eq_soma_m = Equations('''
    I_syn_m  = I_nmda + I_ampa + I_gaba  - I_inj - clamp*i_inj    : amp
    ...
    mossy_eqs += eq_soma_m
    mossy_eqs += voltage_clamp(v_cmd = -70 *mV)
...
#    spiketimes = [(ii, tspike) for ii in xrange(N_pre)]
    ...
    syn_NMDA = Synapses(P0, mossy, model = nmda_eqs, pre = 'x += wNMDA', implicit=True, freeze=True)
    mossy.s_nmda = syn_NMDA.j
    syn_NMDA[:, :]        = True
    syn_NMDA.wNMDA[:, :]  = 1.0
    ...
    syn_AMPA = Synapses(P0, mossy, model = ampa_eqs, pre = 'h += wAMPA', implicit=True, freeze=True)
    mossy.s_ampa = syn_AMPA.y
    syn_AMPA[:, :]        = True
    syn_AMPA.wAMPA[:, :]  = 1.0
    ...
    M5  = StateMonitor(mossy, 'I_gaba', record = True)
    M6  = StateMonitor(mossy, 'I_ampa', record = True)
    M7  = StateMonitor(mossy, 'I_nmda', record = True)
    M8  = StateMonitor(mossy, 'I_syn_m', record = True)
    M9  = StateMonitor(mossy, 'i_inj', record = True)
    M10  = StateMonitor(mossy, 'w', record = True)
    #===================================================================================================================
...
#    if I_injected[i] < 0:
#        print "Input Resistance: " + str((M0[0][22000] * volt - El_m)/(I_injected[i]*pA))
#        print
#        print "Sag ratio: " + str((M0[0][22000]*volt - El_m)/(min(M0[0])*volt - El_m))
#    else:
#        print "Slow AHP: " + str((min(M0[0]) * volt - El_m))
#        print
#
#
#    #peak voltage
#    if I_injected[i] > 0:
#        Vpeak[i] = 1000*(max(M0[0])  - El_m / volt) # in mvolts
#    else:
#        Vpeak[i] = 1000*(min(M0[0])  - El_m/ volt) # in mvolts
...
#    savefig('raster_plots(2).png', bbox_inches='tight', dpi = 1080)
    ...
    for _,t in S.spikes:
        ...
        vm_soma[i] = (El_m + 88*mV)
    plot(M0.times / ms, vm_soma / mV)
    ylabel('V_soma (mV)')
    xlabel('time (ms)')
#    savefig('mossy_Iinj.png', bbox_inches='tight', dpi = 1080)
...
#    savefig('mossyCell.png', bbox_inches = 'tight', dpi = 1080)
##==============================================================================
